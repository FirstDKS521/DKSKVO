...
#import "NSObject+DKSAddKN.h"
#import <objc/runtime.h>
#import <objc/message.h>

/*
 私有实现KVO的真实target类，每一个target对应了一个keyPath和监听该keyPath的所有block，
 当其KVO方法调用时，需要回调所有的block
 */
@interface KSBlockTarget : NSObject

/** 添加一个KVOBlock*/
- (void)ks_addBlock:(void (^)(__weak id obj, id oldValue, id newValue))block;
- (void)ks_addNotificationBlock:(void (^)(NSNotification *notification))block;

- (void)ks_doNotification:(NSNotification *)notification;

@end

@implementation KSBlockTarget {
    //保存所有的block
...
- (void)ks_addBlock:(void (^)(__weak id, id, id))block {
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    ...
    //执行该target下的所有block
    [kvoBlockSet enumerateObjectsUsingBlock:^(void (^block)(__weak id obj, id oldVal, id newVal), BOOL * _Nonnull stop) {
...
@implementation NSObject (DKSAddKN)
static void *const KSKVOBlockKey = "KSKVOBlockKey";

- (void)ks_addObserverBlockForKeyPath:(NSString *)keyPath block:(void (^)(id _Nonnull, id _Nonnull, id _Nonnull))block {
    if (!keyPath || !block) return;
    //取出存有所有KVOTarget的字典
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, KSKVOBlockKey);
    if (!allTargets) {
        //没有则创建
        allTargets = [NSMutableDictionary new];
        //绑定在该对象中
        objc_setAssociatedObject(self, KSKVOBlockKey, allTargets, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    //获取对应keyPath中的所有target
    KSBlockTarget *targetForKeyPath = allTargets[keyPath];
    if (!targetForKeyPath) {
        //没有则创建
        targetForKeyPath = [KSBlockTarget new];
        //保存
    ...
    [targetForKeyPath ks_addBlock:block];
    //对第一次注册KVO的类进行dealloc方法调剂
    [self ks_swizzleDealloc];
}
...
- (void)ks_removeObserverBlockForKeyPath:(NSString *)keyPath{
    ...
    KSBlockTarget *target = allTargets[keyPath];
    if (!target) return;
...
- (void)ks_removeAllObserverBlocks {
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, KSKVOBlockKey);
    if (!allTargets) return;
    [allTargets enumerateKeysAndObjectsUsingBlock:^(id key, KSBlockTarget *target, BOOL *stop) {
        [self removeObserver:target forKeyPath:key];
    }];
    [allTargets removeAllObjects];
}

static void *const KSNotificationBlockKey = "KSNotificationBlockKey";

- (void)ks_addNotificationForName:(NSString *)name block:(void (^)(NSNotification *notification))block {
    if (!name || !block) return;
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, KSNotificationBlockKey);
    if (!allTargets) {
        allTargets = @{}.mutableCopy;
        objc_setAssociatedObject(self, KSNotificationBlockKey, allTargets, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    KSBlockTarget *target = allTargets[name];
    if (!target) {
        target = [KSBlockTarget new];
        allTargets[name] = target;
        [[NSNotificationCenter defaultCenter] addObserver:target selector:@selector(ks_doNotification:) name:name object:nil];
    }
    [target ks_addNotificationBlock:block];
    [self ks_swizzleDealloc];
}
...
- (void)ks_removeAllNotification {
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, KSNotificationBlockKey);
    if (!allTargets.count) return;
    [allTargets enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, KSBlockTarget *target, BOOL * _Nonnull stop) {
        [[NSNotificationCenter defaultCenter] removeObserver:target];
    }];
    [allTargets removeAllObjects];
}

- (void)ks_postNotificationWithName:(NSString *)name userInfo:(NSDictionary *)userInfo {
...
- (void)ks_swizzleDealloc {
    ...
    id newDealloc = ^(__unsafe_unretained id objSelf) {
        //在这里我们移除所有的KVO
        [objSelf ks_removeAllObserverBlocks];
        //移除所有通知
        [objSelf ks_removeAllNotification];
